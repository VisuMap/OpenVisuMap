...
namespace VisuMap.DataLink {
    ...
    public class CmdServer : IDisposable {
        ...
        public CmdServer() {
            app = DataLink.App.ScriptApp;
            cmdPort = int.Parse(app.GetProperty("DataLink.CmdPort", cmdPort.ToString()));
        ...
        public void ServerProc() {
            try {
                serverSkt = new UdpClient(new IPEndPoint(IPAddress.Any, cmdPort));
            ...
            switch (cmd) {
                case CmdRunScript:
                    ...
                    mainForm.Invoke(new MethodInvoker(delegate () {
                        app.RunScript("!" + script, app.LastView);
                    }));
                    Application.DoEvents();
                    break;
                ...
                case CmdGetExpression:
                    ...
                    mainForm.Invoke(new MethodInvoker(delegate () {
                        var retObj = app.RunScript("!vv.Return(" + expression + ");", app.LastView);
                ...
                case CmdLoadTable0: {
                        ...
                        if (string.IsNullOrEmpty(dsName) || (dsName == app.Dataset.Name)) {
                            ...
                            Form parentForm = DataLink.scriptEngine?.ParentForm;
                            ...
                            if ((dsTable != null) && ((frm as IVisuMap)?.Map?.Filter != null) )                               
                                dsTable = dsTable.ApplyFilter((frm as IVisuMap)?.Map?.Filter);
        ...
        public void CheckTcpListener() {
            if (tcpListener == null) {
                tcpListener = new TcpListener(IPAddress.Any, cmdPort);
                tcpListener.Start();
            }
        }
