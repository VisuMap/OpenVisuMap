        ...
        public int intScale(double value) {
            ...
            while (lo <= hi) {
                ...
                if (value < key)
                    ...
                    if ((lo == bins) && (hi == bins)) {
                        // this special case happens when: value == loopup[bins]. We
                        // cannot return bins, since the caller will try to interplate on the 
                        // interval between lookup[index] and lookup[index+1], the latter doesn't exists if index==bins.
                        return bins - 1;
                    } else {
